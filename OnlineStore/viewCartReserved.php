					...
					<table class="table text-center" id="cart">
						<thead class="thead-light">
							<tr>
								...
								<th scope="col">
									<button onclick="removeAllItem()" class="btn btn-sm btn-outline-danger">Remove All</button>
								</th>
							</tr>
						</thead>
						<?php
							$sql = "SELECT DISTINCT shopId FROM `viewcart` WHERE `userId`= $userId";
							...
							while ($rows = mysqli_fetch_assoc($result)) {
								...
								echo '<tbody>
										<tr>
											<th colspan="5" class="text-left">' . $shopName . '</th>
											<td>
												<form action="partials/_manageCartOriginal.php" method="POST">
													<button name="selectShop" class="btn btn-primary">Order</button>
													<input type="hidden" name="shopId" value="' . $shopId . '">
												</form>
											</td>
										</tr>';
								$itemSql = "SELECT * FROM `viewcart` WHERE `userId`= $userId AND `shopId`= $shopId";
