		...
		<div class="card col-lg-12">
			<div class="card-body">
				<table class="table-striped table-bordered col-md-12 text-center">
					...
					<tbody>
						<?php
							$sql2 = "SELECT DISTINCT contact.orderId AS `orderId` FROM contact JOIN orderitems ON contact.orderId = orderitems.orderId JOIN products ON orderitems.productId = products.productId WHERE products.shopId = '$shopId'";	
							$result2 = mysqli_query($conn, $sql2);
							$count = 0;
							while ($row2 = mysqli_fetch_assoc($result2)) {
								$orderId = $row2['orderId'];
								$sql = "SELECT * FROM `contact` WHERE `orderId` = $orderId"; 
						...
						?>
								<script>
									document.getElementById("notempty").innerHTML = '<div class="alert alert-info alert-dismissible fade show" role="alert" style="width:100%"> You have not recieve any message!	</div>';
		...
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
				...
				<div class="modal-body" id="notReply">
					...
					?>
							<script>
								document.getElementById("notReply").innerHTML = '<div class="alert alert-info alert-dismissible fade show" role="alert" style="width:100%"> You have not Reply any message!	</div>';
							</script>
					<?php
						}
					?>
